...
| **late.events** | 속한 1분 창이 닫힌 뒤 도착해 실시간 윈도우가 버린 건수, 초당 값 | 0이 아닌 게 정상(생성기가 10%를 30~60초 늦게 보내고, 그중 창이 이미 닫힌 것이 여기 온다). 급증하면 네트워크 장애 후 밀린 전송 |
...
위 카드 다섯 개는 **가장 최근 `recon.sh` 실행 결과**다 (아래 "마지막 대사" 시각과 **범위**). 실시간으로 바뀌지 않는다.
이 캡처는 `RECON_FROM=2026-09-21T08:31 RECON_TO=2026-09-21T08:34` 로 3분 구간만 대사한 결과다.

| 칸 | 캡처 값 | 뜻 |
|---|---|---|
| 실시간 합계 | 1,153 | 대사 범위의 Redis 분집계 합 |
| 확정 합계 | 1,147 | 같은 범위의 확정 impression 합 (중복·SSAI 정리 후) |
| 차이 / 차이율 | −6 / −0.52% | 확정 − 실시간. **5% 넘으면 주황** — 조사 대상 |
| 정산 금액 | 138,746 KRW | `daily_settlement.amount` 합. 대사 범위와 상관없이 **그날 전체**다 |
...
cmp-1003:  477 − 473 = +13 − 9 + 0
                        │    │   └ 잔차: 위 둘로 설명 안 되는 나머지. 0 이 정상 (0 이면 표시하지 않는다)
                        │    └ SSAI: 실시간이 둘 다 셌고 배치는 하나만 채택
                        └ 지연반영: 실시간 윈도우가 실제로 버렸고(Flink late_dropped) 배치는 셌다
```

- **차이가 0 이 아닌 건 정상이다.** 잔차가 0 이 아니면 그때가 문제다.
  잔차 + 는 아직 안 닫힌 창(→ `flush-windows.sh`)이나 그 구간에 Flink 잡이 없었던 것,
  잔차 − 는 TTL 1시간을 넘긴 재전송이나 archive 잡이 없어 원본이 빠진 것이다.
- 정산은 **확정값**으로 한다. 실시간은 운영 모니터링용이다.
- `cmp-9999` 의 50% 는 워터마크 밀기용 몇 건이라 비율만 커 보이는 것이다. 무시한다.

**5절 표와 숫자가 다른 이유**: 정합성 표는 **최근 30분** 실시간 합계, 대사는 **하루 전체 또는 `RECON_FROM`~`RECON_TO` 구간**(대사 실행 시점)이다.
...
| 대사 차이율 > 5% | 원인 분해 | 지연·SSAI 로 다 설명되면(잔차 0) 정상. 잔차가 있으면 조사 |
